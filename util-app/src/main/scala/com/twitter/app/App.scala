 ...
 * defined in the member `flag`. Flags must be defined in the constructor.
 * Applications should be constructed with modularity in mind, and common
 * functionality should be extracted into mixins.
 *
 * {{{
 ...
 *
 * Because App extends DelayedInit, the constructor will not be called unless
 * the main(Array[String]) method is called.
 */
trait App extends DelayedInit with Closable with CloseAwaitably {
  ...
  private val constructors = mutable.Buffer[() => Unit]()
  ...
  final def main(args: Array[String]) {
    flag.parseOrExit1(args)

    constructors.foreach(_())

    // Add a help flag by default
    val helpFlag = flag("help", false, "Show this help")
    if (helpFlag()) {
      flag.throwUsage()
    }
